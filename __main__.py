...
class VulnType(str, Enum):
    ...
    SQL_INJECTION = "SQL Injection in SQLDatabaseChain"
    PATH_TRAVERSAL = "Path Traversal"
    DYNAMIC_FUNCTION_EXECUTION = "Dynamic Function Execution"
    SENSITIVE_DATA_IN_MEMORY = "Sensitive Data in Memory"
    IMPROPER_OUTPUTPARSER_VALIDATION = "Improper OutputParser Validation"
    AMAZON_COMPREHEND_MODERATION = "Amazon Comprehend Moderation Chain"
    LAYERUP_SECURITY_FILTERING = "Layerup Security Chain Filtering"
    PRESIDIO_DATA_ANONYMIZATION = "Presidio Data Anonymization"
    RCE_IN_PALCHAIN = "RCE in PALChain"
    DYNAMIC_CHAIN_EXECUTION = "Dynamic Chain Execution"
    SANDBOX_ENFORCEMENT = "Sandbox Enforcement"
    LOGICAL_FALLACY_VALIDATION = "Logical Fallacy Chain Validation"
    CONSTITUTIONAL_CHAIN_VALIDATION = "Constitutional Chain Logical Validation"
    API_KEY_PROTECTION = "API Key and Credential Protection"
    RATE_LIMITING_ENFORCEMENT = "Rate Limiting Enforcement"
    ROLE_BASED_ACCESS_CONTROL = "Role-Based Access Control (RBAC)"
...
class RepoOps:
    def __init__(self, repo_path: Path | str ) -> None:
        ...
        patterns = [
            # LangChain imports
            r'from\s+langchain\.\w+\s+import\s+\w+',  # 특정 모듈 import
            r'from\s+langchain_community\.\w+\s+import\s+\w+',  # 특정 모듈 import
            r'import\s+langchain\.\w+',               # langchain 전체 import

            # Chain and Agent initialization
            r'langchain\.\w+Chain\(',                 # Chain 클래스 생성
            r'langchain\.\w+Agent\(',                 # Agent 클래스 생성
            r'langchain\.tools\.\w+\(',               # Tools 사용
            r'from\s+langchain\.chains\.\w+\s+import\s+\w+',  # Chain import
            r'from\s+langchain\.agents\.\w+\s+import\s+\w+',  # Agent import
            
            # LLM Initialization
            r'langchain\.llms\.\w+\(',                # LLM 클래스 생성
            r'from\s+langchain\.llms\.\w+\s+import\s+\w+',  # LLM 모듈에서 특정 LLM import

            # Vectorstores and Memory
            r'langchain\.vectorstores\.\w+\(',        # 벡터 저장소 생성
            r'langchain\.memory\.\w+\(',              # Memory 클래스 생성
            r'from\s+langchain\.memory\.\w+\s+import\s+\w+',  # Memory 관련 import

            # Prompt templates and examples
            r'langchain\.prompts\.\w+\(',             # 프롬프트 템플릿 사용
            r'from\s+langchain\.prompts\.\w+\s+import\s+\w+',  # Prompt 관련 import

            # LangChain Agent setup
            r'initialize_agent\(',                    # Agent 초기화
            r'create_tool\(',                         # LangChain 도구 생성

            # Utility functions
            r'from\s+langchain\.schema\s+import\s+\w+',  # Schema 관련 import
            r'langchain\.schema\.\w+\(',              # Schema 클래스 호출
            r'langchain\.document_loaders\.\w+\(',    # 문서 로더 사용
